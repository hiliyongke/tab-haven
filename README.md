...
> **本地优先、无账号、无云端的标签工作台**：把散乱的标签组织成有秩序的空间，并保证你永远找得回来。
...
| 数据 | 导出导入（JSON 版本化往返无损 + URL 列表）、从 Tabstead 单向迁移 |
...
TabHaven 采用分层架构：**领域逻辑（core）零 chrome/DOM 依赖，是唯一需要单测的层**；platform 层是唯一触碰 `chrome.*` 的边界；多层形态（sidepanel / popup / full）共享同一套 core 与状态层，每个入口只是不同的 React 根。
...
MIT（继承自前身 Tabstead 的开源许可精神；本产品代码为全新设计与实现）。
