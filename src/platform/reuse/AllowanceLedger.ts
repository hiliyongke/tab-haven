 ...
 *
 * 持久化（MV3 SW 回收防护）：onMutate 快照回调 + restore 合并恢复。
 * 调用方（persistedLedger）把快照镜像到 chrome.storage.session，
 * SW 被回收后重新拉起时经 restore 合并未过期令牌，消除
 * 「授权发放与消费之间 SW 回收导致令牌丢失 → 恢复的标签被误合并」的边缘时序。
 */
